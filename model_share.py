...
class Bias(nn.Module):
    ...
    def forward(self,x):
        x += self.bias.repeat(tuple(list(x.shape[:-1])+[1]))
        return x
...
class VAE(nn.Module):
    ...
    def __init__(self, config_model):
        ...
        self.traj_embedder = nn.Linear(self.num_grid, self.sum_state_size, bias=False)
        self.week_embedder = nn.Linear(self.max_week, self.sum_state_size, bias=False)
        self.hour_embedder = nn.Linear(self.max_hour, self.sum_state_size, bias=False)

        self.enc_traj_bias = Bias(self.sum_state_size)
        self.enc_week_bias = Bias(self.sum_state_size)
        self.enc_hour_bias = Bias(self.sum_state_size)
        ...
        self.dec_traj_bias = Bias(self.sum_state_size)
        self.dec_week_bias = Bias(self.sum_state_size)
        self.dec_hour_bias = Bias(self.sum_state_size)

        self.transformer_decoder = tx.modules.TransformerDecoder(
                # tie word embedding with output layer
                output_layer=self.traj_embedder.weight.t(),
    ...
    def forward(self, data_batch, kl_weight: float) -> Dict[str, Tensor]: # bs,max_pos,4
        ...
        input_traj_embed = self.enc_traj_bias(self.traj_embedder(input_traj_.float()))
        input_hour_embed = self.enc_hour_bias(self.hour_embedder(input_hour_.float()))
        input_week_embed = self.enc_week_bias(self.week_embedder(input_week_.float()))
    ...
    def _embed_fn_transformer(self,
        ...
        if tokens.shape != (args.batch_size,):
            ...
            output_traj_ = torch.zeros((output_traj.shape[0],output_traj.shape[1], self.num_grid), device=self.device)\
                                .scatter_(2,output_traj,1)
        ...
        output_traj_embed = self.dec_traj_bias(self.traj_embedder(output_traj_.float()))
        output_hour_embed = self.dec_hour_bias(self.hour_embedder(output_hour_.float()))
        output_week_embed = self.dec_week_bias(self.week_embedder(output_week_.float()))
