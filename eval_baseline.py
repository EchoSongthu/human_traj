...
class IndividualEval(object):
    ...
    def __init__(self, data):
        ...
        self.X = []
        self.Y = []
        for grid_id in range(1,self.max_locs+1): #每个格点id的经纬度
    ...
    def get_topk_visits_bs(self,trajs, k):
        topk_visits_loc = []
        topk_visits_freq = []
        for traj in trajs:
            traj = np.array([i[0]-1 for i in traj])
    ...
    def get_overall_topk_visits_freq_bs(self, trajs, k):
        _, topk_visits_freq = self.get_topk_visits_bs(trajs, k)
        mn = np.mean(topk_visits_freq, axis=0)
        return mn / np.sum(mn)
    ...
    def get_distances_bs(self, trajs):
        distances = []
        for traj in trajs:
            seq_len = len(traj)
            traj = [i[0]-1 for i in traj]
    ...
    def get_durations_bs(self, trajs):
        d = []
        zero_time = trajs[0][0][1]
        for traj in trajs:
            traj = [[i[0]-1,i[1]] for i in traj]
            hours = (traj[-1][1]-traj[0][1]) / 3600
            num = 0.
            for i, lc in enumerate(traj[1:]):
                if lc[1] == zero_time:
                    num = 0.
                    break
                hour = (lc[1]-traj[i][1])/3600
                num += hour
                if lc[0] != traj[i][0]:
                    d.append(num/hours)
                    num = 0.
    ...
    def get_periodicity_bs(self, trajs):
        """
        stat how many repetitions within a single trajectory
        :param trajs:
        :return:
        ...
        for traj in trajs:
            traj = [i[0]-1 for i in traj]
            len_ = len(traj)
            reps.append(float(len(set(traj)))/len_)
        reps = np.array(reps, dtype=float)
        return reps
    ...
    def get_geogradius_bs(self, trajs):
        """
        get the std of the distances of all points away from center as `gyration radius`
        :param trajs:
        :return:
    ...
    def get_individual_jsds(self, t1, t2):
        ...
        d2 = self.get_distances_bs(t2) # shape = 天数,47
        ...
        l2 =  CollectiveEval.get_visits_bs(t2,self.max_locs)
        ...
        f2 = self.get_overall_topk_visits_freq_bs(t2, 100)
...
class CollectiveEval(object):
    ...
    def get_visits(trajs,max_locs):
        ...
        for traj in trajs:
            for t in traj:
                visits[t] += 1
        # for id_,traj in trajs[1:]:

        visits = visits / np.sum(visits)
        return visits

    @staticmethod
    def get_visits_bs(trajs,max_locs):
        visits = np.zeros(shape=(max_locs), dtype=float)
        for traj in trajs:
            traj = [i[0]-1 for i in traj]
            for t in traj:
...
def evaluate(datasets):
    ...
    test_data = pickle.load(open('/data2/songyiwen/human_traj_/dataset_generation/test_data.pkl', 'rb'))
    # gene_data = pickle.load(open('/data2/songyiwen/human_traj_/dataset_generation/exp2/gen_data_epoch500.pkl', 'rb'))
    gene_data = pickle.load(open('/data2/songyiwen/human_traj_/dataset_generation/timegeo_gen_7.pkl', 'rb'))
