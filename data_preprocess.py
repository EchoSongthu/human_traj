...
class myDataset(object):
    ...
    def __getitem__(self, id):
        ...
        idx = 1 # 0为start token
        for j in range(len(self.split_data[id])): # 天数*24
            for i in range(24):
                if idx < self.max_pos + 1:
                    x = self.split_data[id][j][i][0]
                    y = self.split_data[id][j][i][1]
                    item[idx][0] = x * self.max_grid +y +1 # grid id
                    item[idx][1] = i +1 # hour
                    item[idx][2] = self.split_data[id][j][i][2] +1 # y # week
                    idx += 1
                else:
                    break
